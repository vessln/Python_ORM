...
def find_authors_nationalities():
    ...
    for a in authors_with_nationality:
...
def delete_review_by_id(r_id):
    reviewer = Review.objects.get(id=r_id)
    reviewer.delete()
...
def filter_authors_by_nationalities(nat):
    matching_authors = (Author.objects.filter(nationality=nat).order_by("first_name", "last_name"))
    result = []
...
def filter_authors_by_birth_year(year1, year2):
    matching_authors = (Author.objects.filter(
                        birth_date__year__range=(year1, year2)).order_by("-birth_date")
                        )
    result = [f"{a.birth_date}: {a.first_name} {a.last_name}" for a in matching_authors]
